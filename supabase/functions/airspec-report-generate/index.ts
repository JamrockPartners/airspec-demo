...
TOOLTIP RULE: Every data-bound mark (bar, line, point, circle, arc, area) MUST include a "tooltip" encoding channel — an array of field definitions with "title" and optional "format". Tooltips give users hover-readable context for each mark. Example:
  encoding { x:{...}, y:{...}, tooltip: [{ field:"category", type:"nominal", title:"Category" }, { field:"value", type:"quantitative", title:"Value", format:{type:"number",maximumFractionDigits:0} }] }
  For temporal fields in tooltips, use format:{type:"date",dateStyle:"medium"}.

COMMON PATTERNS:
  Bar chart:     mark "bar", encoding { x:{field,type:"nominal"}, y:{field,type:"quantitative",aggregate:"sum"}, tooltip:[{field,type:"nominal",title:"Category"},{field,type:"quantitative",title:"Value"}] }
  Line chart:    mark "line" (object with point:true), encoding { x:{field,type:"temporal" or "nominal"}, y:{field,type:"quantitative"}, tooltip:[{field,type:"temporal",title:"Date"},{field,type:"quantitative",title:"Value"}] }
  Stacked bar:   mark "bar", encoding { x:{field,type:"nominal"}, y:{field,type:"quantitative",aggregate:"sum",stack:"zero"}, color:{field,type:"nominal"}, tooltip:[{field,type:"nominal",title:"Category"},{field,type:"nominal",title:"Segment"},{field,type:"quantitative",title:"Value"}] }
  Donut/arc:     mark "arc" (object with innerRadius), encoding { theta:{field,type:"quantitative"}, color:{field,type:"nominal"}, tooltip:[{field,type:"nominal",title:"Category"},{field,type:"quantitative",title:"Count"}] }
  Scatter:       mark "circle", encoding { x:{field,type:"quantitative"}, y:{field,type:"quantitative"}, color:{field,type:"nominal"}, size:{field,type:"quantitative"}, tooltip:[{field,type:"nominal",title:"Group"},{field,type:"quantitative",title:"X"},{field,type:"quantitative",title:"Y"}] }
    ...
    { "id": "salesChart", "type": "chart", "datasetId": "salesBreakdown", "title": "Sales breakdown",
      "graphicBinding": { "parameter": "chartMode", "cases": [
        { "equals": "bar", "value": { "mark": { "type": "bar", "color": "#3264D6", "cornerRadiusEnd": 4 },
          "encoding": { "x": { "field": "category", "type": "nominal", "title": "Category" }, "y": { "field": "value", "type": "quantitative", "title": "Value" }, "tooltip": [{ "field": "category", "type": "nominal", "title": "Category" }, { "field": "value", "type": "quantitative", "title": "Value", "format": { "type": "number", "maximumFractionDigits": 0 } }] },
